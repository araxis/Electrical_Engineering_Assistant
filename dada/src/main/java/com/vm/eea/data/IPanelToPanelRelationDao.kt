...
interface IPanelToPanelRelationDao {
    ...
    @Query("SELECT * FROM panel_panel_relations where toPanelId= :panelId")
    fun getFeederRelationByConsumerId(panelId:Long):Flow<PanelToPanelRelationEntity>

    @Query("SELECT * FROM panel_panel_relations where id= :relationId")
    fun getFeederRelationById(relationId:Long):Flow<PanelToPanelRelationEntity>
    ...
    suspend fun updateLength(relationId:Long,value:Double,unit: UnitOfLength)
