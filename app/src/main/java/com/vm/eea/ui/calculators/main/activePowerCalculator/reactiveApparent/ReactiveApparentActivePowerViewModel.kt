...
class ReactiveApparentActivePowerViewModel(private val calculator: ActivePowerCalculator
                                           ):ContainerHost<ReactiveApparentState,Nothing> ,ViewModel() {

    override val container: Container<ReactiveApparentState, Nothing>
         =container( ReactiveApparentState())
    ...
    private fun isValid(state: ReactiveApparentState):Boolean{
        if(state.reactivePowerField.notValid) return false
        if(state.apparentPower.notValid) return false
        return true
    }

    private fun calculate(inputState: ReactiveApparentState)=intent{
